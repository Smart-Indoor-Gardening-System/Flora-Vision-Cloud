...
interface EventBody {
 ...
 const editDevice = async (event: any, context: any): Promise<any> => {
   ...
   if (!event.body) {
     ...
     const expressionAttributeValues : any = {};
      ...
      if (plantName) {
         updateExpression += ' plantName = :plantName,';
         expressionAttributeValues[':plantName'] = plantName;
     }
      if (description) {
	 ...
	 if (plantType) {
		 updateExpression += 'plantType = :plantType,';
		 expressionAttributeValues[':plantType'] = plantType;
		}
...
export const handler = addCorsResHeaders(editDevice);
